...
void WriteFile(CImage& img, const char* filename)
{
	...
	if (dot==0) {
		...
		sprintf(newfilename,"%s.tga",filename);
	} 
	else {
    ...
    {
        if (img.PixType() == typeid(uchar))
            WriteFileTGA(*(CByteImage *) &img, newfilename);
        else
           throw CError("ReadFile(%s): haven't implemented conversions yet", newfilename);
    }
    else
        throw CError("WriteFile(%s): file type not supported", newfilename);
	free(newfilename);
